...
Message stats over a period (`24h` | `7d` | `30d`, default `24h`). Add an optional legacy `groupId` for one WhatsApp group, or repeat `groupIds` to aggregate multiple groups. The response's `groupBreakdown` still lists all active groups. The dashboard's multi-select also expands saved custom-group presets into these group IDs. `summary.interactions` counts distinct session-and-chat pairs for API clients: one or 100 messages in the same chat through the same WhatsApp session count as one, not as a message or participant count. The dashboard's **Active groups** card is derived from `groupBreakdown`, counts each WhatsApp group with activity once, and excludes direct chats. `byTypeBreakdown` separates sent, received, total, delivered-recipient, and read-recipient counts for every message content type. Its recipient counts deduplicate a person within each chat and type, then add the chat totals. `summary.reactedMessages` counts outgoing messages with active emoji reactions and `summary.emojiReactions` counts the active user reactions on them. Each `topChats` row also reports sent, received, total-message, delivered/read recipient, and last-active values for API clients. Each row's delivered/read counts are distinct people during the selected period. The summary adds the per-chat counts, so a person present in multiple selected groups contributes once to each group. ADMIN key required.
